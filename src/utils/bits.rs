...
pub fn get_bit<T: Integer + std::ops::Shr<usize, Output = T> + From<u8>>(value: T, bit_index: usize) -> bool {
    ...
    log!("UTILS", format!("{0:#0width$b}[{1}] ({0}) = {2}", value, bit_index, bit as u8, width = bit_size(value) + 1));
    return bit;
}
...
pub fn set_bit<T: Integer + std::ops::BitOr<usize, Output = T>>(old_value: T, bit_index: usize) -> T {
    ...
    log!("UTILS", format!("{0:#0width$b}[{2}] ({0}) => {1:#0width$b} ({1})", old_value, new_value, bit_index, width = bit_size(old_value) + 1));
    return new_value;
}
...
pub fn assign_bit<T: Integer + std::ops::Shl<usize, Output = T> + From<u8>>(old_value: T, bit_index: usize, status: bool) -> T {
    ...
    log!("UTILS", format!("{0:#0width$b}[{2}] ({0}) = {3} => {1:#0width$b} ({1})", old_value, new_value, bit_index, status as u8, width = bit_size(old_value) + 1));
    return new_value;
}
...
fn check_carry_add<T: Integer + std::ops::Shl<usize, Output = T> + From<u8>>(value: T, operand: T, index: usize) -> bool {
    ...
    log!("UTILS", format!("{0} + {1} ({0:#0width$b} + {1:#0width$b}) would carry on bit {2} ? {3}", value, operand, index, result, width = bit_size(value) + 1));
...
fn check_carry_sub<T: Integer + From<u8> + std::ops::Shl<usize, Output = T>>(value: T, operand: T, bit_index: usize) -> bool
    ...
    log!("UTILS", format!("{0} + {1} ({0:#0width$b} - {1:#0width$b}) would carry on bit {2} ? {3}", value, operand, bit_index, result, width = bit_size(value) + 1));
